...
struct MailView: View {
    ...
    func render(_ template: String) -> String { let parts = Calendar.current.dateComponents([.year, .month], from: store.month); let values = ["{recipient}": store.settings.recipient, "{year}": "\(parts.year ?? 0)", "{month}": "\(parts.month ?? 0)", "{sheet_url}": store.settings.sheetURL, "{dropbox_url}": store.settings.dropboxURL, "{income_total}": "\(store.incomeTotal())", "{expense_total}": "\(store.expenseTotal())"]; return values.reduce(template) { $0.replacingOccurrences(of: $1.key, with: $1.value) } }
