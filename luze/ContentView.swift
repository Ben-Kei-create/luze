...
struct SummaryStep: View {
    @EnvironmentObject var store: AppStore
    var body: some View { let d = store.current; let vpass = d.transactions.filter { $0.decision == .expense }.reduce(0) { $0 + $1.amount }; let transport = d.officeDays * store.settings.oneWayFare * 2; Page(title: "4. 集計") { HStack(spacing: 40) { Metric(title: "収入", value: d.incomeTotal); Metric(title: "支出合計", value: store.expenseTotal()) }; VStack(spacing: 8) { ExpenseLine(title: "固定費", value: store.settings.rent); ExpenseLine(title: "Vpass経費", value: vpass); ExpenseLine(title: "交通費", value: transport) }.frame(maxWidth: 360); HStack { CountChip(title: "経費", count: d.transactions.filter { $0.decision == .expense }.count, color: .green); CountChip(title: "除外", count: d.transactions.filter { $0.decision == .excluded }.count, color: .gray); CountChip(title: "保留", count: d.transactions.filter { $0.decision == .pending }.count, color: .orange) }; StatusRow(title: "Google Sheets", ok: d.sheetExported, detail: d.sheetExported ? "反映済み" : "未反映"); Button(store.isWorking ? "反映中…" : "内容を確認してGoogle Sheetsへ反映") { Task { await store.exportToSheet() } }.disabled(store.isWorking); HStack { Button("戻る") { store.step = 3 }; Spacer(); Button("メールへ") { store.complete(4); store.selection = .mail }.buttonStyle(.borderedProminent) } } }
}
